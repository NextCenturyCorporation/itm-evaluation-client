# coding: utf-8

"""
    ITM TA3 API

    This is the specification of the TA3 API for In The Moment (ITM).  Currently, the Evaluation API for TA2 supports functionality for the Phase 2 Evaluations.  The API is based on the OpenAPI 3.0.3 specification.

    The version of the OpenAPI document: 0.5.3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from swagger_client.p2triage.models.base_state import BaseState

class TestBaseState(unittest.TestCase):
    """BaseState unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> BaseState:
        """Test BaseState
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `BaseState`
        """
        model = BaseState()
        if include_optional:
            return BaseState(
                unstructured = '',
                elapsed_time = 10,
                meta_info = swagger_client.p2triage.models.meta_info.MetaInfo(
                    scene_id = '', 
                    probe_response = swagger_client.p2triage.models.probe_response.ProbeResponse(
                        scenario_id = '', 
                        probe_id = '', 
                        choice = '', 
                        justification = '', ), ),
                events = [
                    swagger_client.p2triage.models.event.Event(
                        unstructured = '', 
                        type = 'change', 
                        source = '', 
                        object = '', 
                        when = 1.337, 
                        action_id = '', 
                        relevant_state = [
                            'threat_state.threats[gunshots].threat_type'
                            ], )
                    ],
                threat_state = {"unstructured":"Gunshots have been reported in the surrounding area","threats":[{"type":"gunfire","severity":"moderate"}]},
                characters = [
                    swagger_client.p2triage.models.character.Character()
                    ],
                scenario_complete = True
            )
        else:
            return BaseState(
                unstructured = '',
                characters = [
                    swagger_client.p2triage.models.character.Character()
                    ],
        )
        """

    def testBaseState(self):
        """Test BaseState"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
